...
if ($('.navbar-nav').length > 0) {
    ...
    console.log("Navbar does not exist");
}


            var currentURL = window.location.href.split('/').pop(); // This will get the page name like "historic.html"
            console.log(currentURL, "UUUUUUUUUUUUu");

            // Remove the active class from any previously active nav link
    $('.navbar-nav .nav-link.active').removeClass('active');

    // Get the current page's URL and extract the last segment (filename)
    let currentPage = window.location.href.split('/').pop();

    // Iterate through all nav links to find a match
    $('.navbar-nav .nav-link').each(function(){
        // If the href attribute of the link matches the current page, add the active class
        console.log($(this).attr('href'), "HH");
...
function makeRequest(url=base_url+"/api/historic/historic/all/?column_name=Shp.status&search_query=Available") {
